#!/usr/bin/env bash
# =========================================================
# minipost 一键部署（Docker 统一版 · 强化版 · 镜像加速 & Git容错）
# 说明：
#  - 自动检测/安装 Docker 与 compose 插件；Docker 守护进程优化（日志滚动、systemd cgroup、live-restore、镜像加速）
#  - 对齐《1keyintall.txt》执行清单：ZRAM/Swap、THP、BBR+sysctl、ulimits、安全基线、UFW 放行等
#  - Git 仓库容错：目录存在且非 Git 时自动备份重建；远端不匹配时可交互改远端
#  - 交互开关：二次覆盖/备份/内核调优/ulimits/fail2ban/自动安全更新/镜像加速器
# 适配：Ubuntu 24.04 LTS（root 运行）
# =========================================================
...
retry(){ # retry 3 "cmd ..."
  local times="$1"; shift; local n=0 rc=0
  while :; do
    set +e; eval "$@"; rc=$?; set -e
    [ $rc -eq 0 ] && return 0
...
ensure_env_var(){ # 确保 .deploy.env 有键值（不存在则追加）
  ...
  if [ ! -f "$f" ]; then echo "# minipost 部署环境（自动生成）" > "$f"; chmod 600 "$f"; fi
  if ! grep -q "^${k}=" "$f" 2>/dev/null; then echo "${k}=${v}" >> "$f"; fi
}
merge_daemon_json(){ # 将 key:value 合并进 /etc/docker/daemon.json（需要 jq）
  local key="$1" value="$2" f="/etc/docker/daemon.json"
  install -d -m 0755 /etc/docker
  [ -f "$f" ] || echo '{}' > "$f"
  tmp="$(mktemp)"; jq "$key = $value" "$f" > "$tmp" && cat "$tmp" > "$f" && rm -f "$tmp"
}
...
: "${AUTO_ZRAM_SWAP:=yes}"
: "${SWAP_SIZE_GB:=8}"
: "${DO_BACKUP_DATA:=yes}"                   # 启动前备份 data/ 目录（如存在）
: "${DOCKER_MIRROR_URL:=}"                   # 你自定义的镜像加速器（逗号/空格分隔）
# —— 默认镜像加速器候选（自动探测可用性，依次加入）——
...
info "OS: $(lsb_release -ds 2>/dev/null || head -n1 /etc/os-release)  Kernel: $(uname -r)"
...
# ---------- Docker 守护进程优化 + 镜像加速（含默认镜像） ----------
probe_mirror(){
  # 成功条件：HTTP < 500（常见 200/401/403 都算可达）
  local url="$1" code
  code="$(curl -m 3 -fsSIX GET "$url/v2/" -o /dev/null -w '%{http_code}' || true)"
  [ -n "$code" ] && [ "$code" -lt 500 ]
}
build_mirrors_json(){
  # 组合用户自定义 + 默认镜像里可达的，转 JSON 数组
  local all=() u
  # 用户自定义（逗号/空格分隔）
  if [ -n "$DOCKER_MIRROR_URL" ]; then
    IFS=', ' read -r -a user_mirrors <<< "$DOCKER_MIRROR_URL"
    for u in "${user_mirrors[@]}"; do [ -n "$u" ] && all+=("$u"); done
  fi
  # 默认候选
  ...
  for m in "${all[@]}"; do
    [[ " $seen " == *" $m "* ]] && continue
    seen="$seen $m"
    if probe_mirror "$m"; then uniq+=("$m"); fi
  done
  # 生成 JSON
  printf '%s\n' "${uniq[@]}" | jq -R . | jq -s .
}
if command -v jq >/dev/null 2>&1; then
  ...
  if [ "$(echo "$MIRRORS_JSON" | jq 'length')" -gt 0 ]; then
    ...
    yellow "⚠ 未发现可用镜像加速器（可稍后在 /etc/docker/daemon.json 手工配置）"
  fi
...
if [ -d "$BASE_DIR" ] && [ ! -d "$BASE_DIR/.git" ]; then
  ...
  # 若远端不匹配，提示设置为 REPO_URL
  ORIGIN="$(git -C "$BASE_DIR" remote get-url origin || true)"
  if [ -n "$ORIGIN" ] && [ "$ORIGIN" != "$REPO_URL" ]; then
    yellow "当前仓库 origin 与 REPO_URL 不一致："
    echo "  origin:   $ORIGIN"
    echo "  REPO_URL: $REPO_URL"
    if confirm "是否将 origin 改为 REPO_URL 并强制同步？" Y; then
      git -C "$BASE_DIR" remote set-url origin "$REPO_URL" || true
    fi
  fi
...
if [ "${DO_BACKUP_DATA}" = "yes" ] && [ -d "${BASE_DIR}/data" ]; then
  ...
  info "✔ 数据已备份：${BACKUP_ROOT}/data.tgz"
fi
...
COMPOSE_PROFILES="$COMPOSE_PROFILES" docker compose -f "$COMPOSE_FILE" up -d || {
  red "✘ 启动失败"; docker compose -f "$COMPOSE_FILE" ps; docker compose -f "$COMPOSE_FILE" logs --tail=200; exit 1; }

# ---------- 健康检查 ----------
wait_healthy(){
  local svc="$1" timeout="${2:-240}" cid status start=$(date +%s)
  ...
  while :; do
    ...
    case "$status" in healthy|running) green "✔ $svc 健康：$status"; return 0;; esac
...
# ---------- 管理员初始化（可选） ----------
if confirm "是否尝试初始化管理员账号（若后端提供脚本）？" N; then
  set +e
