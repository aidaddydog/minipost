#!/usr/bin/env bash
# =========================================================
# minipost 一键部署（零交互 · 快速就绪 · 公网IP展示 · Adopt接管）
# - 零交互最佳默认；自动安装&优化 Docker；只写入可达的镜像加速器
# - 自动选择标准/Adopt（.repo）模式；不动你非Git目录的现有文件
# - 取消“长时间等待 backend healthy”，改为端口就绪 + 迁移重试
# - 末尾自动打印公网IP访问地址
...
[ "$(echo "$MIRRORS_JSON" | jq 'length')" -gt 0 ] && merge_daemon_json '.["registry-mirrors"]' "$MIRRORS_JSON"
...
EOF
  ...
  sysctl -w vm.vfs_cache_pressure=50 >/devnull 2>&1 || true
fi
...
[ "${DO_BACKUP_DATA}" = "yes" ] && [ -d "${BASE_DIR}/data" ] && { install -d -m 0755 "$BACKUP_ROOT"; tar -czf "${BACKUP_ROOT}/data.tgz" -C "${BASE_DIR}" data; }
...
# ---------- 快速就绪：等待 backend 端口监听（不等 health） ----------
wait_backend_port(){
  local tries="${1:-90}" i=0
  local py='import socket,sys,time
for _ in range(1):
  try:
    s=socket.socket(); s.settimeout(1); s.connect(("127.0.0.1",8000)); s.close(); print("ok"); sys.exit(0)
  except Exception as e:
    sys.exit(1)'
  while [ $i -lt $tries ]; do
    if docker compose -f "$COMPOSE_FILE" exec -T backend python -c "$py" >/dev/null 2>&1; then
      green "✔ backend 端口已监听"
      return 0
    fi
    i=$((i+1)); sleep 2
  done
  yellow "⚠ backend 端口未在预期时间内就绪，继续后续步骤"
  return 1
}
wait_backend_port 90 || true
