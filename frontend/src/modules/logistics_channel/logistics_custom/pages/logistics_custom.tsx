...
export default function LogisticsCustomPage() {
  const [openNew, setOpenNew] = React.useState(false);
  const [openRename, setOpenRename] = React.useState(false);
  const [openDelete, setOpenDelete] = React.useState(false);

  // TODO: 接到真实接口 /api/logistics/custom/list
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold">自定义物流</h2>
        <div className="flex gap-2">
          <button className="rounded bg-black text-white px-3 py-1.5 text-sm" onClick={() => setOpenNew(true)}>
            新增
          </button>
          <button className="rounded border px-3 py-1.5 text-sm" onClick={() => setOpenRename(true)}>
            重命名
          </button>
          <button className="rounded border px-3 py-1.5 text-sm" onClick={() => setOpenDelete(true)}>
            删除
          </button>
        </div>
      </div>
      ...
      <div className="border rounded-xl p-4 bg-white">
        <div className="text-sm text-slate-500">（列表占位）这里呈现渠道列表，列头/按钮位置与旧 UI 一致。</div>
      </div>
      ...
      <LDialog
        ...
        footer={
          <div className="flex justify-end gap-2">
            <button className="px-3 py-1.5 text-sm rounded border" onClick={() => setOpenNew(false)}>
              取消
            </button>
      ...
      <LDialog
        ...
        footer={
          <div className="flex justify-end gap-2">
            <button className="px-3 py-1.5 text-sm rounded border" onClick={() => setOpenDelete(false)}>
              取消
            </button>
