...
export function LayerProvider({ children }: { children: React.ReactNode }){
  ...
  useEffect(()=>{
    ...
    if(!el){
      el = document.createElement("div"); el.id = "layer-root"; el.setAttribute("data-layer-host", "true")
  ...
  const api: Ctx = {
    ...
    update: (id, visible) => setEntries(m => {
      const next = new Map(m); const e = next.get(id); if(e) next.set(id, { ...e, visible }); return next
    }),
