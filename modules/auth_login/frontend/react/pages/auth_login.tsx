...
export default function AuthLoginPage(){
  ...
  async function submit(e: React.FormEvent){
    ...
    try{
      ...
      if(!res.ok){
        const text = await res.text(); throw new Error(text || "登录失败")
      }
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] grid place-items-center">
      <form onSubmit={submit} className="w-[min(92vw,400px)] border rounded-xl p-6 space-y-4 bg-white">
        ...
        <input className="border rounded px-3 py-2 w-full" placeholder="用户名" value={username} onChange={e=>setUsername(e.target.value)} required />
        <input type="password" className="border rounded px-3 py-2 w-full" placeholder="密码" value={password} onChange={e=>setPassword(e.target.value)} required />
