...
export function LayerProvider({ children }: { children: React.ReactNode }){
  ...
  const counts = useMemo(()=>{
    ...
    entries.forEach(e => { if(e.visible) c[e.kind]++ })
    return c
  }, [entries])
  ...
  const api: Ctx = {
    container,
    register: (kind, visible) => { const id = idRef.current++; setEntries(m => new Map(m).set(id, { id, kind, visible })); return id },
    update: (id, visible) => setEntries(m => { const n=new Map(m); const e=n.get(id); if(e) n.set(id, { ...e, visible }); return n }),
...
export function useRegisterLayer(kind: Kind, visible: boolean){
  ...
  useEffect(()=>{ idRef.current = register(kind, visible); return ()=>{ if(idRef.current!=null) unregister(idRef.current) } }, [])
