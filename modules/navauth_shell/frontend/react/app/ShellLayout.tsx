...
export function ShellLayout(){
  ...
  React.useEffect(()=>{ fetchNav().then(d => setItems(d.items || [])) }, [])

  const activeL1 = items.find(it => loc.pathname.startsWith((it.href || it.path || "").replace(/\/$/, "")))
            ...
            {items.map(it => {
              ...
              return (
                <Link key={href} to={href}
                  className={ "px-3 py-1.5 rounded " + (active ? "bg-slate-900 text-white" : "text-slate-700 hover:text-slate-900 hover:bg-slate-100") }>
                  {it.title || href}
                </Link>
              )
            })}
          ...
          <div className="border-t bg-white">
            <div className="mx-auto max-w-screen-xl px-6 h-10 flex items-center gap-2">
              {activeL1.children.map(c => {
                ...
                return (
                  <Link key={href} to={href}
                    className={"px-2 py-1 rounded text-sm " + (active ? "bg-slate-900 text-white" : "text-slate-700 hover:bg-slate-100 hover:text-slate-900")}>
                    {c.title || href}
                  </Link>
                )
              })}
