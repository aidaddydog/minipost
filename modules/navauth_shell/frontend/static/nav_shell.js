...
/* ====== 由后端注入的初始化路径（MINIPOST_INIT） ====== */
const MINIPOST_INIT = (typeof window!=='undefined' && window.MINIPOST_INIT) ? window.MINIPOST_INIT : {};
    ...
    // —— 初始化覆盖（由 MINIPOST_INIT 提供）——
    if(MINIPOST_INIT.locked_path) lockedPath = MINIPOST_INIT.locked_path;
    if(MINIPOST_INIT.locked_sub)  lockedSubHref = MINIPOST_INIT.locked_sub;
    if(MINIPOST_INIT.locked_tab)  lockedTabHref = MINIPOST_INIT.locked_tab;
      // 三级
    ...
    /* ===== 首屏：默认进入当前状态对应的内容 ===== */
    \1
      if(subHref==='/logistics/channels'){
        if (typeof window !== 'undefined' && window.LOGI && typeof window.LOGI.renderList === 'function'){
          const type = (lockedTabHref||'').split('/').pop() || 'platform';
          window.LOGI.renderList(type);
        }else{
          tabCard.classList.remove('no-tabs');
          tableWrap.classList.add('hidden');
          logsTableWrap.classList.add('hidden');
          footerBar.classList.add('hidden');
          tabPanel.innerHTML = '物流渠道：请加载 /static/logistics/logistics_channel.js 以渲染列表。';
        }
        return;
      }
