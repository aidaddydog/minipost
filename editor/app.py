...
# -*- coding: utf-8 -*-
"""
独立前端模版编辑器（FastAPI）——v2
- 端口：默认 6006（EDITOR_PORT 可改；如需 <1024 端口，容器需 NET_BIND_SERVICE 能力）
- 登录：EDITOR_USER / EDITOR_PASS（默认：daddy / 20240314AaA#）
- 范围：扫描 backend/templates、backend/static、frontend/src、frontend/public、editor（可编辑编辑器自身）
- 元数据：支持中文名（cn）+ 中文释义（desc），可在 UI “信息”按钮在线维护，写入 editor/config/templates_map.yaml
- 保存即生效：保存即写入磁盘；对 Jinja/静态资源通常请求即生效（如后端禁用模板自动重载，可在 docker-compose 的 backend 增加 JINJA_AUTO_RELOAD=1）
"""
...
from fastapi.responses import HTMLResponse, JSONResponse, PlainTextResponse, StreamingResponse, RedirectResponse, FileResponse
...
# 允许的后缀（含 .py：可编辑自身后端代码；预览不支持 .py）
ALLOWED_SUFFIX = [".html",".htm",".css",".js",".mjs",".ts",".tsx",".svg",".xml",".py"]

# 可编辑/扫描的目录白名单（相对 ROOT_DIR）
SCAN_DIRS = [
    ROOT_DIR / "backend" / "templates",
    ROOT_DIR / "backend" / "static",
    ROOT_DIR / "frontend" / "src",
    ROOT_DIR / "frontend" / "public",
    ROOT_DIR / "editor",  # 允许编辑器自我编辑（模板/CSS/py 等）
]

# 备份与导出目录（放在仓库 editor 子目录，便于版本化）
...
app.mount("/static", StaticFiles(directory=str(ROOT_DIR / "editor" / "static")), name="static")
templates = Jinja2Templates(directory=str(ROOT_DIR / "editor" / "templates"))
...
def _list_backups(rel: str):
    ...
    for p in items:
        t = p.stem
        try:
            dt = datetime.strptime(t, "%Y%m%d%H%M%S").strftime("%Y-%m-%d %H:%M:%S")
        except Exception:
            dt = t
...
@app.post("/api/save")
async def api_save(payload: dict, ok: bool = Depends(ensure_login)):
    ...
    # 立即生效：对静态/模板类文件，写盘后通常请求立刻生效；这里返回 ok 即可
...
def api_preview(path: str, ok: bool = Depends(ensure_login)):
    """生成临时预览文档；HTML 直接原样返回；JS/CSS/SVG 生成简单壳页面引用；.py 不支持预览"""
    fp = (ROOT_DIR / path).resolve()
    if not _is_allowed(fp): raise HTTPException(403, "path not allowed")
    if not fp.exists(): return HTMLResponse("<!doctype html><body>空白</body>")
...
def api_export_one(path: str, ok: bool = Depends(ensure_login)):
    ...
    return FileResponse(path=str(fp), filename=filename, media_type=mt)

# ========== 路由：新建文件/目录 ==========
@app.post("/api/new")
def api_new(payload: dict, ok: bool = Depends(ensure_login)):
    rel = payload.get("path","").strip().lstrip("/")
    content = payload.get("content","")
